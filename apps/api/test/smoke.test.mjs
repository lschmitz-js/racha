...
test('emergency self-service flow + PII never leaks', async () => {
  ...
  // Phones are normalized on write: a verified number takes its country's shape,
  // and a local number from a country we can't infer is stored exactly as typed.
  await api(`/api/emergency/${admin.token}`, {
    method: 'PUT',
    body: JSON.stringify({ contact_name: 'Mom', contact_phone: '(604) 555 1234', player_phone: '+55 11 91234-5678' }),
  });
  let saved = await (await api(`/api/emergency/${admin.token}`)).json();
  assert.equal(saved.contact.contact_phone, '+1 604-555-1234');
  assert.equal(saved.contact.player_phone, '+55 11 91234 5678');

  await api(`/api/emergency/${admin.token}`, {
    method: 'PUT',
    body: JSON.stringify({ contact_name: 'Mom', contact_phone: '11 91234-5678' }),
  });
  saved = await (await api(`/api/emergency/${admin.token}`)).json();
  assert.equal(saved.contact.contact_phone, '11 91234-5678', 'a DDI-less foreign number is never made +1');

  const csv = await (await api('/api/players/emergency-export', { headers: M })).text();
  assert.ok(csv.includes('11 91234-5678'), 'CSV export carries the stored number');
